...
def pid_control():
    ...
    while running:          
        ...
        # If mode changed, reset PID state and encoder diff baseline
        ...
        if not use_PID:
            ...
            if current_movement == 'forward' or current_movement == 'backward':
                ...
                # Only apply *turning PID* when BOTH wheels are commanded, opposite signs (true in-place turn)
                if left_pwm != 0 and right_pwm != 0 and (left_pwm * right_pwm < 0):
                    # Equalize magnitudes: error is |L| - |R|
                    ...
                    # Adjust magnitudes in opposite directions, preserving signs
                    left_sign  = 1.0 if left_pwm  >= 0 else -1.0
                    right_sign = 1.0 if right_pwm >= 0 else -1.0
                    target_left_pwm  = left_pwm  - correction * left_sign   # reduce if left too fast, increase if too slow
                    target_right_pwm = right_pwm + correction * right_sign  # increase if right too slow, reduce if too fast
                else:
                    # Pivot (one side zero) or ambiguous: pass-through, but keep PID state reset from mode change
                    pass
